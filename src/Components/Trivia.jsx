...
const preguntas = [
  ...
  {
    "pregunta": "¿Cuál es una característica destacada de las laptops MSI para gamers?",
    "respuestas": [
...
const Trivia = () => {
  ...
  const [preguntasSeleccionadas] = useState(() => getRandomQuestions(preguntas, 3));
  ...
  if (vista >= 3) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-black text-white ">
        ...
        <p className="text-2xl mb-8">Respuestas correctas: <span className="font-bold">{score}</span> de 3</p>
        <button
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-black text-white">
      <div className="bg-[#222]   p-8 rounded-3xl shadow-lg w-4/5 h-[50%]   text-center mb-8">
        <h2 className="text-7xl font-bold mb-32">{preguntaActual.pregunta}</h2>
        <div className="flex flex-col gap-20">
          {preguntaActual.respuestas.map((resp, idx) => (
            <div
              key={idx}
              className={`cursor-pointer py-4 px-6 rounded-2xl text-6xl border-2 border-[#DD0209] transition-all
                ${respuestaSeleccionada === idx
                  ? idx === preguntaActual.correcta
                    ? "bg-green-600"
                    : "bg-red-600"
                  : "bg-[#111] hover:bg-[#DD0209] hover:text-white"}
                ${respuestaSeleccionada !== null && respuestaSeleccionada !== idx ? "opacity-60" : ""}
              `}
              onClick={() => respuestaSeleccionada === null && handleRespuesta(idx)}
            >
              {resp}
            </div>
          ))}
